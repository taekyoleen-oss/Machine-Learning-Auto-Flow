 ...
 * 입력층(LoadData.parameters.fileContent)만 채운다 — Phase 4 URL 로더와 동일 원리.
 */
...
/**
 * 데이터를 Supabase Storage 'datasets' 버킷에 업로드해 "웹 예제"로 등록한다(Phase C).
...
/** 데이터 로더로 취급할 모듈 타입(앱별 LoadData/XolLoading 등). */
const LOADER_TYPES = new Set(["LoadData", "XolLoading"]);
