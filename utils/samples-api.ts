...
export const samplesApi = {
  ...
  // 샘플 생성
  async create(sample: CreateSampleRequest): Promise<{ id: number; message: string }> {
    try {
      const response = await fetch(`${API_BASE}/api/samples`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to create sample: ${response.status}`);
      }
  ...
  async importFromFile(file: File): Promise<{ id: number; message: string }> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        throw new Error(errorData.error || `Failed to import sample: ${response.status}`);
      }
  ...
  // 샘플 수정
  async update(id: number, sample: UpdateSampleRequest): Promise<{ message: string }> {
    try {
      const response = await fetch(`${API_BASE}/api/samples/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        throw new Error(errorData.error || `Failed to update sample: ${response.status}`);
      }
  ...
  async delete(id: number): Promise<{ message: string }> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        throw new Error(errorData.error || `Failed to delete sample: ${response.status}`);
      }
